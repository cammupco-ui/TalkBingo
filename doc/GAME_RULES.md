...
*   **보상**:
    *   **공격 성공 (Steal)**: 도전자가 타일을 빼앗고 **+10 GP** 획득.
    *   **방어 성공 (Defense)**: 원래 소유자가 타일을 지키고 **+5 GP** 획득.
...
빙고 줄을 1줄 완성하고 게임을 종료하면 해당 유저의 승리가 되며 GP를 획득합니다.
...
*   **결과**: 유저 A 승리.
*   **보상**: **유저 A** = 1줄 GP(+20) + 승리 GP(+20)

#### **시나리오 2: 연승 (이어하기)**
*   유저 A가 첫 번째 줄 완성 (이어하기 선택) -> 유저 A는 일단 **빙고 줄 GP(+20)**만 획득.
*   **더블 포인트**: 유저 A가 두 번째 줄도 완성하고 종료 시 -> **"더블 GP"** 획득.
*   **트리플 포인트**: 유저 A가 세 번째 줄까지 완성하고 종료 시 -> **"트리플(3배) GP"** 획득.

#### **시나리오 3: 무승부 (Draw)**
*   1차: 유저 A가 첫 줄 완성.
*   2차: 유저 B가 두 번째 줄 완성하고 게임 종료.
*   **결과**: 무승부. 승리 GP를 **반반(50%)씩** 나눠 가집니다.
*   **보상**:
    *   **유저 A**: 1줄 GP(+20) + 승리 GP 50%(+25)
    *   **유저 B**: 2줄 GP(+40) + 승리 GP 50%(+25)

#### **시나리오 4: 역전승 (Comeback)**
*   1차: 유저 A가 첫 줄 완성.
*   2차 & 3차: 유저 B가 두 번째와 세 번째 줄을 연속으로 완성하고 게임 종료.
*   **보상**:
    *   **유저 A**: 1줄 GP(+20) (승리 GP 없음)
    *   **유저 B**: 2줄 GP(+40) + 3줄 GP(+60) + **"더블 GP"** (2,3차 승리분)
...
포인트는 **GP(게임 활동 보상)**와 **VP(유료 화폐)**로 분리됩니다.

| 포인트 | 약어 | 설명 | 획득 방법 | 사용처 |
| :--- | :--- | :--- | :--- | :--- |
| **Game Point** | **GP** | 게임 활동으로 획득하는 **누적 포인트**. 소비 불가. | 셀 획득 **+1**, 빙고줄 **+20/+40/+60**, 챌린지 공격 **+10**, 방어 **+5**, 승리 **+20** | 프로필 누적 통계 (향후 랭킹/칭호 확장) |
| **Value Point** | **VP** | **현금 구매** 또는 **보상형 광고**로 획득. | IAP 구매, 보상형 광고 시청 **+5 VP** (일일 최대 10회) | 광고 제거 (**25 VP**/게임, **8,000 VP**/영구) |
| **Trust Score** | **TS** | **신뢰도 평가**. (1~5점) | 게임 종료 후 평가 | 신뢰도 표시 |

> **Note**:
> *   **GP**는 소비되지 않고 영구 누적됩니다. 유저의 게임 활동량을 나타내는 지표입니다.
> *   **VP**는 유료 화폐이며, 현금 결제 또는 보상형 광고(30초)로 획득합니다.
> *   기존 VP/AP/EP → VP 전환 규칙은 **폐지**되었습니다.
...
*   **계정 연동 (Link Account)**: 설정(Settings) 메뉴에서 구글/이메일 계정을 연동하면, 현재까지 획득한 **모든 포인트(GP/VP)와 게임 기록**이 안전하게 서버에 영구 저장이 되며, 기기가 변경되어도 유지됩니다.
