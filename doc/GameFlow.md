...
3.  **쿼리 로직** (`GameSession.fetchQuestionsFromSupabase`):
    ...
    *   **밸런스 알고리즘**: 후보군 중 **총 50개**를 추출합니다. (실제 사용 25개 + 예비 25개)
        *   **Main**: 진실 13개 + 밸런스 12개 (총 25개, 보드 배치용)
        *   **Reserve**: 진실 13개 + 밸런스 12개 (총 25개, 교체/미니게임 보상용)
...
2.  **전송 데이터 구조**:
    ...
    // game_sessions.game_state 내부
    {
      "cells": [
        { 
          "index": 0,
          "main_q": { "id": "T26-001", "content": "..." }, 
          "reserve_q": { "id": "B26-088", "content": "..." } 
        },
        ... 25 items
      ],
